...
def test_home_preview_is_read_only_bounded_and_does_not_create_db(tmp_path):
    ...
    store = ScheduleStore(path)
    base = datetime(2026, 9, 24, 12, tzinfo=UTC).timestamp()
    store.reminder("2026-10-01T15:00:00+00:00", "Review notes", now=base)
    store.reminder("2026-10-02T15:00:00+00:00", "Prepare quiz", now=base)
    store.timer(30, "Not a reminder", now=base)
    count, preview = read_pending_reminder_preview(path=path, limit=1, now=base)
    assert count == 2
